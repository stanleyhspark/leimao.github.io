...
import argparse
import numpy as np
from OpenAI_MCPG_FC import OpenAI_REINFORCE_FC
...
def Train_Model(env = env):
    ...
    AI_player = OpenAI_REINFORCE_FC(num_actions = num_actions, num_features = num_features, rand_seed = RAND_SEED, mode = 'train')
    ...
    while episode < EPISODE_MAX:
        ...
        while True:  
            ...
            if done:
                ...
                AI_player.Clear_Episode_Replays()
    ...
    env.close()
            ...
            AI_player.REINFORCE_FC_Train(action = action, reward = reward, observation = observation, terminal = done)
